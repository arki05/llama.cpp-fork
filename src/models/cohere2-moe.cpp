...
void llama_model_cohere2_moe::load_arch_tensors(llama_model_loader &) {
    ...
    GGML_ASSERT(hparams.n_expert > 0 && "n_expert must be > 0 for COHERE2_MOE");
    GGML_ASSERT(hparams.n_expert_used > 0 && "n_expert_used must be > 0 for COHERE2_MOE");
...
llama_model_cohere2_moe::graph::graph(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ...
    for (int il = 0; il < n_layer; ++il) {
        // SWA layers use RoPE, full-attention layers have no positional embedding,
        // except the leading dense layers which use full attention with RoPE
        const bool use_rope = hparams.is_swa(il) || (uint32_t) il < hparams.n_layer_dense_lead;
